...
const createTask = (task) => { //interpolación
    return `<li class="${task.completed?'completed':''}">
                <input type="checkbox" id="${task.id}" ${task.completed?'checked':''}> ${task.name}
            </li>`;
}

const renderTasks = (tasks) => { //pinta las tareas que nos han dado
    taskList.innerHTML='';
...
const handleClick = (e) => { //añade tarea nueva al array y la pinta
    e.preventDefault();
